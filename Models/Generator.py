...
def init_generator():
    ...
    concat_layer = tf.keras.layers.Concatenate()([malware, noise])
    # assert concat_layer.output_shape(None, 2381)

    # Hidden layer of size 3952
    dense_layer1 = tf.keras.layers.Dense(3952, activation='relu')(concat_layer)
    ...
    gen = tf.keras.Model(inputs=[malware, noise], outputs=output)
    return gen
